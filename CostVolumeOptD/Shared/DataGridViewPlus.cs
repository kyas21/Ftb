...
{
    class DataGridViewPlus : System.Windows.Forms.DataGridView
    {
        ...
        // ----------------------------------------------------------------------------------
        // DataGridViewのカラム幅をファイルへ保存／読込するための実装 Start
        // ----------------------------------------------------------------------------------
        /// <summary>
        ...
        // ----------------------------------------------------------------------------------
        // DataGridViewのカラム幅をファイルへ保存／読込するための実装 End
        // ----------------------------------------------------------------------------------
        ...
        // ----------------------------------------------------------------------------------
        // DataGridViewの各カラムへの入力可能文字を制限するための実装 Start
        // ----------------------------------------------------------------------------------
        /// <summary>
        ...
        // ----------------------------------------------------------------------------------
        // DataGridViewの各カラムへの入力可能文字を制限するための実装 End
        // ----------------------------------------------------------------------------------
        ...
        // DataGridViewでCtrl-Iキー押下時に行挿入するための実装 Start
        // ----------------------------------------------------------------------------------
            ...
            else if ((e.Modifiers & Keys.Control) == Keys.Control)
            {
                switch (e.KeyCode)
                {
                    case Keys.V:
                        ...
                        {
                            // 最後のNULL行をコピーするかどうか
                            if (r >= lines.GetLength(0) - 1 && "".Equals(lines[r]) && nflag == false) break;
                            ...
                            {
                                // セルが存在しなければ貼り付けない
                                if (!(x + c2 >= 0 && x + c2 < dgv.ColumnCount && y + r >= 0 && y + r < dgv.RowCount))
                                {
                                    continue;
                                }
                                // 非表示セルには貼り付けない
                                ...
                                {
                                    _editingColumn = x + c2;
                                    byte[] cc = Encoding.GetEncoding("SHIFT-JIS").GetBytes(vals[c].Substring(i, 1));
                    ...
                    case Keys.D:
                        dgv.Rows.RemoveAt(y);
                        /*
                        foreach (DataGridViewRow dRow in dgv.SelectedRows)
                        {
                            if (!dRow.IsNewRow)
                            {
        ...
        // DataGridViewでCtrl-Vキー押下時にクリップボードから貼り付けるための実装↑↑↑↑↑↑
        // DataGridViewでDelやBackspaceキー押下時にセルの内容を消去するための実装↑↑↑↑↑↑
        // DataGridViewでCtrl-Iキー押下時に行挿入するための実装 End
        // ----------------------------------------------------------------------------------
